...
cdef class IntuitiveFuzzy(object):
	...
	cdef long int num_attr
	cdef long int num_objs
	cdef dict relational_matrices




	def __init__(self, np.ndarray  dataframe, list att_nominal_cate):
	...
	cpdef dict _get_single_attr_IFRM(self, np.ndarray data):
		"""
		...
		"""
		cdef dict result = {}
		cdef list list_index_real
		cdef double mu, v
		cdef np.ndarray rel_matrix, column
		cdef int k
	...
	cpdef np.ndarray _get_multiple_attr_IFRM(self, list attributes):
		"""
		...
		cdef np.ndarray combined, result
	...
	cpdef np.ndarray _get_union_IFRM(self, np.ndarray IFRM_1, np.ndarray IFRM_2):
		"""
		...
		cdef np.ndarray result = np.empty((self.num_objs, self.num_objs), dtype=tuple)
		cdef int i,j
	...
	cpdef float _get_cardinality(self, np.ndarray IFRM):
		"""
		...
		caridnality = sum([round((1 + x[0] - x[1])/2,2) for x in np.ravel(IFRM)])
		return caridnality
	
	cpdef float intuitive_partition_dist(self, list p1, list p2):
		"""
		...
		if (len(p1) == 0):
			...
			return round(self.intuitive_partition_dist_d(IFRM_1),2)

	cpdef float sig_start(self, list B, a):
		"""
	...
	cpdef float intuitive_partition_dist_d(self, np.ndarray IFRM):
		"""
			...
			Returns :
				- result : A scalar representing the distance
		"""
		cdef float IFRM_d_cardinality,IFRM_cardinality
		cdef np.ndarray IFRM_d 
	...
	cpdef float sig(self, np.ndarray IFRM, a):
		"""
			This function measures the significance of an attribute a to the set of 
	...
	cpdef float condition_stop (self, np.ndarray IFRM_1, np.ndarray IFRM_2):
		cdef np.ndarray IFRM_1_d, IFRM_2_d, IFRM_condition
		cdef float sup_m, suy_v
		cdef int i, j
	...
	cpdef list filter(self, bint verbose=False):
		"""
		...
		"""
		# initialization 
		cdef float SIG_B_c, condition
		cdef double  start, finish 
		cdef list B = []
		cdef list W = []
		cdef float d = self.intuitive_partition_dist(B, B + [self.attributes[-1]])
		cdef float D = self.intuitive_partition_dist(self.C, self.attributes)		
		cdef np.ndarray matrix_C = self._get_multiple_attr_IFRM(self.C)
		cdef np.ndarray IFRM_TG
		...
		while (condition >= 1 - self.ro and d > D):
			...
			if(verbose):
				...
				print(f'    --> Selected c_m = {c_m}')
				print(f'    --> Distance from B -> (B union d) : {round(d,2)}\n')
